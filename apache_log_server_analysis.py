# Databricks notebook source exported at Thu, 15 Sep 2016 18:17:42 UTC
# MAGIC %md
...
not200DF = logs_df.filter(logs_df.status!=200).orderBy(desc("status"))
not200DF.show(10)
...
# TODO: Replace <FILL IN> with appropriate code
unique_host_count = logs_df.agg(countDistinct(logs_df.host)).collect()[0][0]
...
days_with_hosts = []
hosts = []
for i in range(len(daily_hosts_df.select('count').collect())):
    hosts.append(daily_hosts_df.select('count').collect()[i][0])
    days_with_hosts.append(daily_hosts_df.select('day').collect()[i][0])
...
total_req_per_day_df = logs_df.select(logs_df['host'],dayofmonth(logs_df['time']).alias('day'))
total_req_per_day_df = total_req_per_day_df.groupBy('day').count()
total_req_per_day_df=total_req_per_day_df.select('day',total_req_per_day_df['count'].alias('total'))
total_req_per_day_df=total_req_per_day_df.join(daily_hosts_df, 'day','inner')
avg_daily_req_per_host_df = total_req_per_day_df.select(total_req_per_day_df['day'], (total_req_per_day_df['total']/total_req_per_day_df['count']).alias('avg_reqs_per_host_per_day'))
...
for i in range(len(avg_daily_req_per_host_df.select('day').collect())):
    avgs.append(avg_daily_req_per_host_df.select('avg_reqs_per_host_per_day').collect()[i][0])
    days_with_avg.append(avg_daily_req_per_host_df.select('day').collect()[i][0])
print(days_with_avg)
print(avgs)
...
top_20_not_found_df = top_20_not_found_df.orderBy(desc("count"))
...
hosts_404_count_df = not_found_df.groupby('host').count()
hosts_404_count_df = hosts_404_count_df.orderBy(desc('count'))
...
errors_by_date_sorted_df = not_found_df.select(dayofmonth(not_found_df['time']).alias('day')).groupBy('day').count()
...
errors_404_by_day = []
for i in range(len(errors_by_date_sorted_df.select('day').collect())):
    errors_404_by_day.append(errors_by_date_sorted_df.select('count').collect()[i][0])
    days_with_errors_404.append(errors_by_date_sorted_df.select('day').collect()[i][0])
...
top_err_date_df = errors_by_date_sorted_df.orderBy(desc('count'))
...
hour_records_sorted_df = hour_records_sorted_df.orderBy('hour',ascending=True)
...
for i in range(len(hour_records_sorted_df.select('hour').collect())):
    not_found_counts_per_hour.append(hour_records_sorted_df.select('count').collect()[i][0])
    hours_with_not_found.append(hour_records_sorted_df.select('hour').collect()[i][0])
